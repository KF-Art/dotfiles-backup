...
function ble/syntax:bash/extract-command-by-noderef {
  ...
  [[ ! ${wtype//[0-9]} ]] && ((wtype==_ble_ctx_CMDI||wtype==_ble_ctx_ARGI||wtype==_ble_ctx_ARGVI||wtype==_ble_ctx_ARGEI)) || return 1
  ...
  while ble/syntax/tree#next-sibling "$ret" wvars; do
    ...
    if ((wtype==_ble_ctx_ARGI||wtype==_ble_ctx_ARGVI||wtype==_ble_ctx_ARGEI)); then
      ble/array#push comp_words "${_ble_syntax_text:wbeg:wlen}"
      [[ $opts == *:treeinfo:* ]] &&
        ble/array#push tree_words "$ret"
    fi
...
function ble/syntax/progcolor/word:default/.highlight-filename {
  ...
  if ((ext==142)); then
    ...
    ble/syntax/progcolor/word:default/.highlight-pathspec-with-attr "$_ble_attr_ERR"
  elif (((wtype==_ble_ctx_RDRF||wtype==_ble_ctx_RDRD||wtype==_ble_ctx_RDRD2)&&count>=2)); then
    ble/syntax/progcolor/wattr#setattr "$p0" "$_ble_attr_ERR"
  elif ((wtype==_ble_ctx_CMDI)); then
    ...
    if local rex='^[0-9]+-?$|^-$'; [[ $value =~ $rex ]]; then
      ble/syntax/progcolor/wattr#setattr "$p0" "$_ble_attr_DEL"
...
function ble/syntax/progcolor/word:default/.is-option-context {
  ((wtype==_ble_ctx_ARGI||wtype==_ble_ctx_ARGEI||wtype==_ble_ctx_ARGVI)) || return 1
  local iword ret
